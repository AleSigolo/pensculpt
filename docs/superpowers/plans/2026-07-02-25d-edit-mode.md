...
3. **Remove** the old selection-flow `.fullScreenCover(isPresented: $vm.showSculptScreen, onDismiss: projectSurfaceStrokes)` modifier, the `sculptButton` view and its ZStack line, the `autoProjectStrokes`/`projectedStrokeIDs` state, and the `projectSurfaceStrokes()` function. In `DrawingViewModel`, remove `var showSculptScreen = false`. Then add the **new** cover for the expand button (SculptScreen shares the `sculptObjects` binding, so no onDismiss handler is needed — mesh/ink edits made there are already in place when it closes):
