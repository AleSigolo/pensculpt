...
> **AS-BUILT NOTE (2026-07-03):** `SurfaceStroke.projectTo2D()` was later deleted as dead code — `StrokeLifter.bake` (Task 4) superseded it as the only 3D→2D projection path and its last callers were its own tests (`testProjectTo2DUsesStrokeColor`, `testProjectTo2DMultipliesColorAlphaByOpacity`, removed with it). The color/opacity-folding conventions above live on in `bake`.
...
> **AS-BUILT NOTE (2026-07-03, ordering-proof source-stroke handoff):** the listing above carries three amendments landed after the original step. (1) Both commit handlers now start with `guard appMode == .select else { return }` — selection commits are no-ops outside select mode. (2) The reset logic shown inline in `toggleMode`/`exitEditMode` was extracted into a private `resetToDrawMode()`, which additionally clears `editSessionStrokes`. (3) The VM gained `private(set) var editSessionStrokes: [Stroke]`, populated from `selectedStrokes` **before** `appMode = .edit` in both handlers. Rationale: `Edit25DOverlay` is constructed by the same body evaluation that first sees `.edit`, and SwiftUI runs `onChange` handlers only after that evaluation — so DrawingScreen's original onChange→`@State editSourceStrokes` copy handed the overlay an empty array; its `onAppear`/`startSession` captured `[]`, `ShapeInflater.sculpt(from: [])` produced an empty mesh, and every fresh lift failed with the "Couldn't lift that selection" toast. Capturing the strokes in the VM before the mode flip makes the handoff ordering-proof at the data level (pinned by `testLassoCommitSetsEditSessionStrokesBeforeModeFlips` / `testSmartSelectCommitSetsEditSessionStrokesBeforeModeFlips` and end-to-end by `LiftSelectionFlowTests`).
...
> **AS-BUILT NOTE (2026-07-03, object lifecycle at commit):** the listing above carries two amendments landed after review. (1) An empty bake (user erased all surface ink) now **discards the object** instead of retaining it with unmatchable `sourceStrokeIDs` — matching the spec's error-handling row "originals are simply removed; object is discarded"; unlifted originals still carry through untouched. (2) Commit ends by **pruning fully-orphaned objects** (no source stroke still on the canvas): such objects can never be re-entered under exact-match semantics and would otherwise accumulate in the document forever. Both happen inside the same undoable operation — the `preSessionObjects` restore covers them.
