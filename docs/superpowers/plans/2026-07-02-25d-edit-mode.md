...
4. Guard the PK sync against mid-session hidden ink (originals are absent from `pkDrawing` during an edit session; persisting that state would corrupt the drawing file). Both persistence paths get the guard — the debounced sync and the explicit flush (which the autosave-toggle `.onChange` and the Save button call):
...
5. Hide the mode-toggle and Save nav buttons during edit — in `navBarItems`, wrap the first and last Buttons (Save would otherwise show "Saved!" while the guarded flush writes nothing):
